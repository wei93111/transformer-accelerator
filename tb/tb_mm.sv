...
`define SDF_FILE "mm_ctrl/mm_ctrl_syn.sdf"
        ...
        forever begin
            ...
            repeat (`AD) begin
                for (row = 0; row < `VL; row = row + 1) begin
                    mtrx_out[(tile_row * `VL + row) * `N + (tile_col * `AD + col)] = acc_data[row * `ACC_W +: `ACC_W];
                end
                col = col + 1;
                #(`CYCLE * 1.0);
            end
            ...
            // increment cnt
            if (tile_col == `N / `AD - 1) begin
                if (tile_row == `M / `VL - 1) begin
                    tile_col = 0;
                    tile_row = 0;
                end else begin
                    tile_col = 0;
